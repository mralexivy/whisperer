...
- **Meeting Notes + Ask AI RAG** — Full offline meeting intelligence: auto-detection of Zoom/Teams/Meet/Webex/Slack, streaming transcription via the existing whisper pipeline, Wax-based semantic Q&A (MiniLM CoreML / ANE, bundled — no download), persisted per-meeting chat history, source citations with one-tap scroll-to-timestamp in transcript. See `Whisperer/Meetings/`.

- **Meeting Notes** — Full-featured offline meeting recorder. Auto-detects Zoom, Teams, Meet, Webex, FaceTime, Slack via `MeetingDetector`. Streams transcript in real time through the same whisper pipeline as dictation. 3-column Meeting Studio UI: library list / transcript+AI-overview / audio player+assistant. AI overview post-meeting (summary, decisions, action items, open questions) via `MeetingAIService.generateOverview()`. Ask AI powered by Wax RAG (`MeetingRAGEngine`): MiniLM CoreML encoder (ANE-capable, bundled — zero download, macOS 14+) chunks transcript into ≤60s semantic blocks with 1-segment boundary overlap, retrieves top-8 chunks per question via hybrid BM25+HNSW search, feeds only those to the LLM. Chat history persisted per meeting as `<uuid>-chat.json`. Source citations under each AI answer: tap any chip to scroll the transcript to that timestamp via `NotificationCenter`.

...
- **Native MCP Server** — Local Model Context Protocol server (`http://localhost:8547/mcp`) so AI tools (Claude Desktop, Cursor, etc.) can query Whisperer data without any cloud relay. HTTP/SSE transport via NWListener + `StatefulHTTPServerTransport` (Swift MCP SDK). Four tools: `list_meetings`, `get_meeting`, `search_transcriptions`, `get_transcription`. All CoreData reads happen on background contexts — no main actor contention. Port and enable/disable configurable in Settings → MCP Server card. Server auto-starts with the app, stops on quit, restarts on port change. Integration tests in `WhispererTests/MCPServerTests.swift`.

- **Dedicated meeting engine set** — MeetingEngines singleton manages the four meeting engines (Nemotron, Whisperer V3, Qwen3.5-4B MTP, Sortformer) with download+warm lifecycle. Removed MiniLM/Wax RAG retrieval; Ask AI now uses whole-transcript + KV cache for faster, more complete answers. Meeting onboarding screen (MeetingPrepView) drives engine preparation from readiness state. Post-stop sequence reordered: cleanup → title → overview.

- **Meeting Notes + Ask AI** — Full offline meeting intelligence: auto-detection of Zoom/Teams/Meet/Webex/Slack, streaming transcription via the existing whisper pipeline, whole-transcript Ask AI with KV-cache prefix (no external index or embedding model), persisted per-meeting chat history, source citations with one-tap scroll-to-timestamp in transcript. See `Whisperer/Meetings/`.

- **Meeting Notes** — Full-featured offline meeting recorder. Auto-detects Zoom, Teams, Meet, Webex, FaceTime, Slack via `MeetingDetector`. Streams transcript in real time through the same whisper pipeline as dictation. 3-column Meeting Studio UI: library list / transcript+AI-overview / audio player+assistant. AI overview post-meeting (summary, decisions, action items, open questions) via `MeetingAIService.generateOverview()`. Ask AI uses the full transcript as the LLM system prompt (cached per meeting via KV cache); `parseCitations` extracts `[Ns]` timestamp patterns from responses into source citation chips. Chat history persisted per meeting as `<uuid>-chat.json`.

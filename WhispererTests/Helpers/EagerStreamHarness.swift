...
func runEagerFixture(
    ...
    //
    // `chunkCollector` also keeps the whole chunk rather than its text: `start` and `end` are the
    // only pause evidence in the pipeline, and throwing them away here is what made the interior
    // boundary class unmeasurable for six benchmark rounds.
    transcriber.onChunkCompleted = { [chunkCollector] chunk in
        chunkCollector.record(chunk)
